...
export default async function KelolaSeleksiPage() {
  ...
  const [{ data: peserta }, { data: prosesList }, { data: lowonganList }] = await Promise.all([
    supabase
      .from("peserta_seleksi")
      .select("id, jenis_seleksi, jalur_pendaftaran, status, seleksi_lowongan_id, created_at")
  ...
  const tahunProses = (prosesId: string) => prosesList?.find((p) => p.id === prosesId)?.tahun;
  const prosesOptions =
    lowonganList?.map((l) => ({
      id: l.id,
      label: `${JENIS_LABEL[l.jenis_seleksi] ?? l.jenis_seleksi} — ${l.jabatan_lowong} (${tahunProses(l.seleksi_proses_id) ?? "?"})`,
    })) ?? [];
        ...
        {peserta?.map((p) => {
          ...
          return (
            <div key={p.id} className="card p-5">
              ...
              <div className="mt-3">
                <label className="text-xs text-slate-400 block mb-1">Posisi &amp; Siklus Seleksi (untuk Tim UKK)</label>
                <TautkanProsesSelect
                  pesertaId={p.id}
                  prosesSaatIni={p.seleksi_lowongan_id}
