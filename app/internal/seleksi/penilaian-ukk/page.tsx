...
export default async function PenilaianUkkPage({
  ...
  await requireRole(["tim_ukk"]);
  const profile = await getSessionProfile();
  const { proses: seleksiProsesId } = await searchParams;

  const supabase = await createClient();
  const [{ data: prosesList }, { data: lowonganList }] = await Promise.all([
    supabase.from("seleksi_proses").select("id, tahun").order("created_at", { ascending: false }),
    supabase.from("seleksi_lowongan").select("id, seleksi_proses_id, jenis_seleksi, jabatan_lowong"),
  ]);

  const prosesOptions =
    prosesList?.map((p) => {
      const posisi = lowonganList?.filter((l) => l.seleksi_proses_id === p.id) ?? [];
      const labelPosisi = posisi
        .map((l) => `${JENIS_LABEL[l.jenis_seleksi] ?? l.jenis_seleksi} — ${l.jabatan_lowong}`)
        .join(" & ");
      return { id: p.id, label: `${labelPosisi || "(belum ada posisi)"} (${p.tahun})` };
    }) ?? [];
  ...
  if (seleksiProsesId) {
    ...
    // Peserta ditautkan ke POSISI (seleksi_lowongan), bukan langsung ke
    // proses — ambil semua posisi milik proses ini dulu, lalu peserta
    // yang tertaut ke salah satu posisi tersebut.
    const lowonganIds = (lowonganList ?? [])
      .filter((l) => l.seleksi_proses_id === seleksiProsesId)
      .map((l) => l.id);

    const { data: pesertaSeleksi } = lowonganIds.length
      ? await supabase
          .from("peserta_seleksi")
          .select("id, user_id")
          .in("seleksi_lowongan_id", lowonganIds)
      : { data: [] };
