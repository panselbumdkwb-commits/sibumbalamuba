...
export default async function DokumenPage() {
  ...
  // Hanya anggota_pansel (role: panitia_seleksi) yang membuat draf
  // surat — ketua_pansel HANYA mengoreksi (menerima & menandatangani,
  // atau menolak & meminta revisi), jadi tombol "Buat Draf Surat"
  // tidak ditampilkan untuknya sama sekali.
  const bisaBuatDraf = profile.role === "panitia_seleksi";

  const supabase = await createClient();
  const { data: dokumen } = await supabase
    .from("dokumen_internal")
    .select("id, judul, status, pembuat_id, approver_id, versi, nomor_surat, jenis_naskah, sifat, lampiran, kepada, isi_surat, tembusan, entitas_seleksi, catatan_revisi, created_at, updated_at")
  ...
  return (
    <main className="p-6 max-w-4xl mx-auto flex flex-col gap-6">
      <PageHeader
        ...
        description={
          isKetua
            ? "Sebagai Ketua Panitia, Anda mengoreksi surat dari anggota panitia: terima & tanda tangani, atau tolak & minta revisi."
            : "Anda bisa membuat draf dan mengajukan surat. Persetujuan/tanda tangan akhir wewenang Ketua Panitia."
        }
      />

      <Link href="/internal/dokumen/arsip" className="text-sm text-primary-700 hover:underline w-fit">
        🗄️ Buka Arsip Surat — cari berdasarkan nomor, perihal, tujuan, atau tanggal
      </Link>

      {bisaBuatDraf && <CreateSuratForm />}
