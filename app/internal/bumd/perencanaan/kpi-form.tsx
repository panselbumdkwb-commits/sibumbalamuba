...
import { tambahKpi } from "@/actions/monev-bumd.actions";
...
const KATEGORI_OPTIONS = [
  ...
  { value: "kontribusi_daerah", label: "Kontribusi Daerah" },
];

export default function KpiForm({ bumdId, tahun }: { bumdId: string; tahun: number }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ kategori: "keuangan", namaIndikator: "", targetNilai: "", satuan: "" });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setForm({ kategori: "keuangan", namaIndikator: "", targetNilai: "", satuan: "" });
    setOpen(false);
    router.refresh();
  }
