import Link from "next/link";
import Navbar from "@/components/navbar";
import Footer from "@/components/footer";
import EntityLogo from "@/components/entity-logo";
import { createClient } from "@/lib/supabase/server";

type SkorEntitasBumd = {
  bumd_id: string;
  nama_bumd: string;
  skor_total: number | null;
  kategori: string | null;
  jumlah_indikator_terhitung: number;
};

type SkorEntitasBlud = {
  blud_id: string;
  nama_blud: string;
  skor_total: number | null;
  kategori: string | null;
  jumlah_indikator_terhitung: number;
};

export default async function PublicLandingPage() {
  const supabase = await createClient();

  const [{ data: bumdList }, { data: bludList }, { data: publikasi }] = await Promise.all([
    supabase.from("bumd").select("id, nama, jenis_usaha, status").order("nama"),
    supabase.from("blud").select("id, nama, jenis_layanan, status").order("nama"),
    supabase
      .from("publikasi_monev")
      .select("jenis_entitas, tahun")
      .eq("dipublikasikan", true)
      .order("tahun", { ascending: false }),
  ]);

  const totalEntitas = (bumdList?.length ?? 0) + (bludList?.length ?? 0);

  // Tahun terbaru yang sudah eksplisit dipublikasikan admin_bpsda,
  // per jenis entitas — BUMD dan BLUD bisa beda tahun evaluasi.
  const tahunBumd = publikasi?.find((p) => p.jenis_entitas === "bumd")?.tahun ?? null;
  const tahunBlud = publikasi?.find((p) => p.jenis_entitas === "blud")?.tahun ?? null;

  const [{ data: skorBumd }, { data: skorBlud }] = await Promise.all([
    tahunBumd
      ? supabase.rpc("skor_kesehatan_bumd_publik", { p_tahun: tahunBumd })
      : Promise.resolve({ data: null }),
    tahunBlud
      ? supabase.rpc("skor_kesehatan_blud_publik", { p_tahun: tahunBlud })
      : Promise.resolve({ data: null }),
  ]);

  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      <Navbar />

      {/* Hero */}
      <section className="relative overflow-hidden text-white">
        <div className="absolute inset-0">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/images/kota-batu-aerial.jpg"
            alt="Pemandangan udara Kota Batu"
            className="h-full w-full object-cover"
          />
          <div className="absolute inset-0 bg-gradient-to-b from-primary-950/90 via-primary-900/85 to-primary-950/95" />
          <div className="absolute inset-0 opacity-[0.06] bg-[radial-gradient(circle_at_1px_1px,white_1px,transparent_0)] [background-size:24px_24px]" />
        </div>
        <div className="relative max-w-6xl mx-auto px-6 py-20 sm:py-28 text-center">
          <span className="badge bg-brand-500/15 text-brand-300 border border-brand-400/30 mb-5">
            Portal Resmi Pemerintah Kota Batu
          </span>
          <h1 className="text-3xl sm:text-5xl font-bold tracking-tight max-w-3xl mx-auto leading-tight">
            Transparansi Tata Kelola BUMD &amp; BLUD Kota Batu
          </h1>
          <p className="mt-5 text-primary-100/90 max-w-xl mx-auto text-sm sm:text-base">
            Satu sistem terpadu untuk monitoring kinerja, evaluasi, dan
            seleksi Direksi, Dewan Pengawas, dan Komisaris — akuntabel,
            aman, dan berpedoman pada regulasi yang berlaku.
          </p>

          <div className="mt-14 grid grid-cols-3 gap-4 max-w-lg mx-auto">
            <Stat value={String(totalEntitas)} label="Entitas Terdaftar" />
            <Stat value={String(bumdList?.length ?? 0)} label="BUMD" />
            <Stat value={String(bludList?.length ?? 0)} label="BLUD" />
          </div>
        </div>
      </section>

      <main className="flex-1">
        <section id="bumd" className="max-w-6xl mx-auto px-6 py-16 scroll-mt-20">
          <SectionHeader
            eyebrow="Transparansi"
            title="Badan Usaha Milik Daerah (BUMD)"
            desc="Entitas usaha daerah yang bergerak mencari keuntungan sekaligus memberi layanan publik, sesuai PP No. 54/2017."
          />
          <EntityGrid items={bumdList} subtitleKey="jenis_usaha" empty="Belum ada data BUMD yang dipublikasikan." hrefPrefix="/bumd" />
        </section>

        <section id="blud" className="max-w-6xl mx-auto px-6 py-16 scroll-mt-20">
          <SectionHeader
            eyebrow="Transparansi"
            title="Badan Layanan Umum Daerah (BLUD)"
            desc="Unit kerja pemda dengan fleksibilitas pengelolaan keuangan untuk layanan publik, sesuai Permendagri No. 79/2018."
          />
          <EntityGrid items={bludList} subtitleKey="jenis_layanan" empty="Belum ada data BLUD yang dipublikasikan." hrefPrefix="/blud" />
        </section>

        <section id="evaluasi" className="max-w-6xl mx-auto px-6 py-16 scroll-mt-20">
          <SectionHeader
            eyebrow="Akuntabilitas"
            title="Evaluasi Kinerja (Monev)"
            desc="Skor kesehatan hasil monitoring & evaluasi kinerja tahunan. Dihitung otomatis dari data KPI yang sudah melalui proses verifikasi berjenjang, dan hanya tampil setelah eksplisit dipublikasikan oleh BPSDA — bukan begitu ada data. Data KPI/realisasi mentah tidak pernah dipublikasikan, hanya skor akhir dan kategorinya."
          />
          <div className="flex flex-col gap-8">
            <ScoreCardGrid
              title="BUMD"
              tahun={tahunBumd}
              items={skorBumd as SkorEntitasBumd[] | null}
              nameKey="nama_bumd"
              idKey="bumd_id"
              hrefPrefix="/bumd"
            />
            <ScoreCardGrid
              title="BLUD"
              tahun={tahunBlud}
              items={skorBlud as SkorEntitasBlud[] | null}
              nameKey="nama_blud"
              idKey="blud_id"
              hrefPrefix="/blud"
            />
          </div>
        </section>

        <section id="seleksi" className="max-w-6xl mx-auto px-6 py-16 scroll-mt-20">
          <SectionHeader
            eyebrow="Karier"
            title="Seleksi Direksi, Dewan Pengawas &amp; Komisaris"
            desc="Pendaftaran terbuka mengikuti jadwal panitia seleksi. Seluruh tahapan — administrasi, psikotes, tes tertulis, uji kompetensi (UKK), presentasi, hingga wawancara — dicatat sistem dan dapat dipantau peserta secara mandiri."
          />
          <div className="card p-6">
            <p className="text-sm text-slate-600 max-w-lg">
              Sudah punya akun peserta atau ingin mendaftar? Gunakan tombol{" "}
              <span className="font-medium">Peserta Seleksi</span> di
              bagian atas atau bawah halaman ini.
            </p>
          </div>
        </section>
      </main>

      <Footer />
    </div>
  );
}

function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div className="rounded-xl bg-white/10 border border-white/10 py-3">
      <p className="text-xl font-bold text-brand-300">{value}</p>
      <p className="text-[11px] text-primary-100/70 mt-0.5">{label}</p>
    </div>
  );
}

function SectionHeader({ eyebrow, title, desc }: { eyebrow: string; title: string; desc: string }) {
  return (
    <div className="mb-8 max-w-2xl">
      <span className="text-xs font-semibold text-brand-600 uppercase tracking-wide">{eyebrow}</span>
      <h2 className="text-2xl font-semibold text-slate-900 mt-1">{title}</h2>
      <p className="text-sm text-slate-500 mt-2 leading-relaxed">{desc}</p>
    </div>
  );
}

function EntityGrid({
  items,
  subtitleKey,
  empty,
  hrefPrefix,
}: {
  items: { id: string; nama: string; status: string; [key: string]: string | null }[] | null;
  subtitleKey: string;
  empty: string;
  hrefPrefix: string;
}) {
  if (!items?.length) {
    return <div className="card p-6 text-sm text-slate-400">{empty}</div>;
  }

  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => (
        <Link
          key={item.id}
          href={`${hrefPrefix}/${item.id}`}
          className="card p-5 hover:shadow-card-hover hover:border-primary-200 transition-all"
        >
          <div className="flex items-start gap-3">
            <EntityLogo nama={item.nama} />
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <p className="font-medium text-slate-900 truncate">{item.nama}</p>
                <span
                  className={`badge shrink-0 ${
                    item.status === "aktif"
                      ? "bg-accent-50 text-accent-700"
                      : "bg-slate-100 text-slate-500"
                  }`}
                >
                  {item.status}
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-1">{item[subtitleKey]}</p>
              <p className="text-xs text-primary-700 mt-2 font-medium">Lihat hasil evaluasi kinerja →</p>
            </div>
          </div>
        </Link>
      ))}
    </div>
  );
}

// Warna kategori kesehatan (AA/A/BBB/BB/B/C, ambang batas di
// skor_ke_kategori_kesehatan, migration 0018) — pola lampu lalu
// lintas yang umum dipakai dashboard kinerja pemda: hijau = sehat,
// kuning = cukup sehat/perlu perhatian, merah = kurang sehat.
const KATEGORI_COLOR: Record<string, string> = {
  AA: "bg-accent-50 text-accent-700",
  A: "bg-accent-50 text-accent-700",
  BBB: "bg-amber-50 text-amber-700",
  BB: "bg-amber-50 text-amber-700",
  B: "bg-red-50 text-red-600",
  C: "bg-red-50 text-red-600",
};

function ScoreCardGrid<T extends { skor_total: number | null; kategori: string | null; jumlah_indikator_terhitung: number }>({
  title,
  tahun,
  items,
  nameKey,
  idKey,
  hrefPrefix,
}: {
  title: string;
  tahun: number | null;
  items: T[] | null;
  nameKey: keyof T;
  idKey: keyof T;
  hrefPrefix: string;
}) {
  return (
    <div>
      <div className="flex items-baseline justify-between mb-4">
        <h3 className="font-semibold text-slate-900">{title}</h3>
        {tahun && <span className="text-xs text-slate-400">Tahun evaluasi {tahun}</span>}
      </div>

      {!tahun || !items?.length ? (
        <div className="card p-6 text-sm text-slate-400">
          Belum ada hasil evaluasi {title} yang dipublikasikan untuk tahun berjalan.
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            <Link
              key={String(item[nameKey])}
              href={`${hrefPrefix}/${String(item[idKey])}`}
              className="card p-5 hover:shadow-card-hover hover:border-primary-200 transition-all"
            >
              <div className="flex items-start justify-between gap-2">
                <p className="font-medium text-slate-900">{String(item[nameKey])}</p>
                {item.kategori && (
                  <span className={`badge shrink-0 ${KATEGORI_COLOR[item.kategori] ?? "bg-slate-100 text-slate-500"}`}>
                    {item.kategori}
                  </span>
                )}
              </div>
              {item.skor_total !== null ? (
                <>
                  <p className="text-2xl font-bold text-slate-900 mt-2">{item.skor_total}</p>
                  <p className="text-xs text-slate-400 mt-1">
                    dari {item.jumlah_indikator_terhitung} kategori indikator terverifikasi
                  </p>
                </>
              ) : (
                <p className="text-sm text-slate-400 mt-2">Data realisasi terverifikasi belum cukup untuk tahun ini.</p>
              )}
            </Link>
          ))}
        </div>
      )}
    </div>
  );
}
