...
// Menautkan peserta ke satu POSISI (seleksi_lowongan) di dalam sebuah
// siklus seleksi — perlu dilakukan panitia supaya Tim UKK tahu peserta
// mana termasuk posisi mana. Tanpa ini, peserta tidak akan tampil di
// dashboard Penilaian UKK.
export async function tautkanProsesSeleksi(input: unknown) {
  ...
  const { error } = await supabase
    .from("peserta_seleksi")
    .update({ seleksi_lowongan_id: parsed.data.seleksiLowonganId })
    .eq("id", parsed.data.pesertaId);
